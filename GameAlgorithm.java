...
public class GameAlgorithm {
	...
	void display(Game g){		
		...
		//write to output.txt
		File file = new File("D:\\Eclipse Workspace\\Assignment2\\output\\"+g.getOutputNum()+".out");
		try {
			if (!file.exists()) {
				file.createNewFile();
			}
	...
	public ArrayList<Node> generateChildren(Node state, Game g){
		...
		ArrayList<Node> children = new ArrayList<Node>();
		 ...
		 //currBoardState[][] = state.getBoardState();
		for(int i=0;i<g.getN();i++)
			for(int j=0; j<g.getN();j++){
				...
				if(currBoardState[i][j].equalsIgnoreCase(".")){
					...
					if(i-1 > -1 && i-1 < g.getN())
						if(currBoardState[i-1][j].equalsIgnoreCase(state.getPlayer()))
							//currBoardState[i-1][j] = g.getPlayer();
							foundRaid = true;
					...
					if(j-1 > -1 && j-1 < g.getN())
						if(currBoardState[i][j-1].equalsIgnoreCase(state.getPlayer()))
							//currBoardState[i][j-1] = g.getPlayer();
							foundRaid = true;
		...
		/*String temp= g.getPlayer();
		g.setPlayer(g.getNotPlayer());
		g.setNotPlayer(temp);*/
